//! Generation of C header files from Rust source.
//!
//! The generator parses a crate's source with [`syn`], collects the items that can
//! cross the FFI boundary, and renders them as a formatted, human-readable C header.
//! It is normally driven from a `build.rs` script:
//!
//! ```no_run
//! use std::path::Path;
//!
...
//! ```
//!
//! Naming is configured through [`TerraffiGeneratorBuilder`]: prefixes, suffixes and
//! a [`Case`] convention per kind of identifier. Types the header would reference but
//! not declare are rejected, so a header that will not compile is never written; see
//! [`TerraffiError::UnresolvedTypes`].
//!
//! [`syn`]: https://docs.rs/syn

#![warn(missing_docs)]
mod case;
/// Parsing of Rust doc comments and rendering them as Doxygen.
pub mod doc;
mod error;
mod items;
mod validate;

pub use error::TerraffiError;
pub use items::CMacro;
use items::*;
pub use validate::{
    UnrepresentableReason, UnrepresentableType, UnrepresentableTypesError, UnresolvedType,
...
/// Configures and constructs a [`TerraffiGenerator`].
///
/// Every option is optional; anything left unset takes its default when
/// [`TerraffiGeneratorBuilder::build`] is called. [`TerraffiGeneratorBuilder::new`]
/// and [`TerraffiGeneratorBuilder::default`] are equivalent.
#[derive(Debug, Default, Clone, PartialEq, Eq)]
pub struct TerraffiGeneratorBuilder {
    /// If true, dependency crates are scanned for type definitions. Defaults to `true`.
    is_scan_dependencies: Option<bool>,
    /// Crate names to exclude from dependency scanning.
    exclude_crates: Vec<String>,
    /// Type names assumed to be declared by an externally-provided header.
    assumed_declared_types: HashSet<String>,
    /// If true, unresolved types are reported as warnings instead of failing generation.
    is_allow_unresolved_types: bool,
    ...
    is_add_std_includes: Option<bool>,
...
impl TerraffiGeneratorBuilder {
    /// Creates a builder with terraffi's default configuration.
    ///
    /// Equivalent to [`TerraffiGeneratorBuilder::default`]. The defaults themselves
    /// are applied by [`TerraffiGeneratorBuilder::build`], so an unset option and
    /// one explicitly set to its default value behave identically.
    pub fn new() -> Self {
        Self::default()
    }

    /// Disables scanning of dependency crates for type definitions.
    ///
    /// Types defined in dependency crates will not be emitted. Any exported item
    /// that references such a type will still name it, so disable this only when
    /// the dependency's types are declared in a separate header.
    pub fn disable_scan_dependencies(mut self) -> Self {
        self.is_scan_dependencies = Some(false);
        self
    }

    /// Excludes a dependency crate from the scan for type definitions.
    ///
    /// Has no effect unless dependency scanning is enabled, which it is by default.
    pub fn exclude_crate(mut self, name: impl Into<String>) -> Self {
        self.exclude_crates.push(name.into());
        self
    }

    /// Treats `name` as already declared, suppressing the unresolved-type error for it.
    ///
    /// Use this for types declared in a hand-written header pulled in with
    /// [`TerraffiGeneratorBuilder::add_include`], which terraffi cannot see.
    pub fn assume_declared(mut self, name: impl Into<String>) -> Self {
        self.assumed_declared_types.insert(name.into());
        self
    }

    /// Downgrades unresolved types from an error to a warning.
    ///
    /// Generation then succeeds even though the header references types it does not
    /// declare, which will not compile unless they are supplied another way. The
    /// references are available from [`TerraffiGenerator::unresolved_types`].
    pub fn allow_unresolved_types(mut self) -> Self {
        self.is_allow_unresolved_types = true;
        self
    }

    /// Sets a comment placed at the top of the header, rendered as a block comment.
    ///
    /// Newlines in `comment` are preserved.
    pub fn header_comment(mut self, comment: impl Into<String>) -> Self {
        self.header_comment = Some(comment.into());
        self
    }

    /// Sets the name used for the `#ifndef`/`#define` include guard.
    ///
    /// Defaults to the crate directory name in upper snake case with an `_H`
    /// suffix, so `my_crate` becomes `MY_CRATE_H`.
    pub fn header_guard(mut self, guard: impl Into<String>) -> Self {
        self.header_guard = Some(guard.into());
        self
    }

    /// Sets a macro name prepended to every function declaration, such as `DLL_API`.
    ///
    /// A platform-detection `#define` block for the macro is emitted after the
    /// include statements, so the header can control DLL import and export.
    pub fn export_macro(mut self, export_macro: impl Into<String>) -> Self {
        self.export_macro = Some(export_macro.into());
        self
    }

    /// Sets whether `<stdint.h>`, `<stddef.h>` and `<stdbool.h>` are included.
    ///
    /// Defaults to `true`. Turning this off leaves the fixed-width integer types
    /// undeclared unless they are provided another way.
    pub fn add_std_includes(mut self, value: bool) -> Self {
        self.is_add_std_includes = Some(value);
        self
    }

    /// Adds an include statement to the header.
    ///
    /// Pass the file name only; the surrounding `#include "..."` is added.
    pub fn add_include(mut self, include: impl Into<String>) -> Self {
        self.additional_includes.push(include.into());
        self
    }

    /// Adds a macro definition, emitted after the export macro block.
    ...
    /// Exports every public `extern "C"` function. This is the default.
    pub fn export_public_functions(mut self) -> Self {
        self.is_export_public_functions = Some(true);
        self
    }

    /// Exports only functions annotated with `#[terraffi_export]`.
    pub fn export_only_annotated_functions(mut self) -> Self {
        self.is_export_public_functions = Some(false);
        self
    }

    /// Exports every public C-compatible type, whether or not it is referenced.
    pub fn export_public_types(mut self) -> Self {
        self.is_export_public_types = Some(true);
        self
    }

    /// Exports only types annotated with `#[terraffi_export]`, plus any type
    /// reachable from an exported item. This is the default.
    pub fn export_only_annotated_types(mut self) -> Self {
        self.is_export_public_types = Some(false);
        self
    }

    /// Sets a prefix for both struct and enum type names.
    ///
    /// Applied before case conversion. Equivalent to calling
    /// [`TerraffiGeneratorBuilder::struct_prefix`] and
    /// [`TerraffiGeneratorBuilder::enum_prefix`] with the same value.
    ...
    /// Sets a prefix for struct type names. Applied before case conversion.
    pub fn struct_prefix(mut self, prefix: impl Into<String>) -> Self {
        self.struct_prefix = Some(prefix.into());
        self
    }

    /// Sets a prefix for enum type names. Applied before case conversion.
    pub fn enum_prefix(mut self, prefix: impl Into<String>) -> Self {
        self.enum_prefix = Some(prefix.into());
        self
    }

    /// Sets a prefix for constant names. Applied before case conversion.
    pub fn constant_prefix(mut self, prefix: impl Into<String>) -> Self {
        self.constant_prefix = Some(prefix.into());
        self
    }

    /// Sets whether enum variant names are prefixed with their enum's type name,
    /// so that `ExampleEnum::Foo` becomes `EXAMPLE_ENUM_FOO`.
    ///
    /// Defaults to `true`. C has no enum namespacing, so unprefixed variants from
    /// different enums can collide.
    pub fn prefix_enum_cases_with_typename(mut self, value: bool) -> Self {
        self.is_prefix_enum_members_with_typename = Some(value);
        self
    }

    /// Sets a suffix for both struct and enum type names.
    ///
    /// Applied before case conversion. Equivalent to calling
    /// [`TerraffiGeneratorBuilder::struct_suffix`] and
    /// [`TerraffiGeneratorBuilder::enum_suffix`] with the same value.
    ...
    /// Sets a suffix for struct type names. Defaults to `"_t"`.
    pub fn struct_suffix(mut self, suffix: impl Into<String>) -> Self {
        self.struct_suffix = Some(suffix.into());
        self
    }

    /// Sets a suffix for enum type names. Defaults to `"_e"`.
    pub fn enum_suffix(mut self, suffix: impl Into<String>) -> Self {
        self.enum_suffix = Some(suffix.into());
        self
    }

    /// Sets a suffix for constant names. Defaults to no suffix.
    pub fn constant_suffix(mut self, suffix: impl Into<String>) -> Self {
        self.constant_suffix = Some(suffix.into());
        self
    }

    /// Sets the suffix for the discriminant enum generated alongside a tagged union.
    ///
    /// Defaults to `"Kind"`, so `ExampleUnion` gains an `ExampleUnionKind` discriminant
    /// enum. Applied before case conversion.
    pub fn discriminant_enum_suffix(mut self, suffix: impl Into<String>) -> Self {
        self.discriminant_enum_suffix = Some(suffix.into());
        self
    }

    /// Sets the case convention for type names. Defaults to [`Case::Snake`].
    pub fn typename_case(mut self, case: Case) -> Self {
        self.typename_case = Some(case);
        self
    }

    /// Sets the case convention for function parameter names. Defaults to
    /// [`Case::Snake`].
    pub fn parameter_case(mut self, case: Case) -> Self {
        self.parameter_case = Some(case);
        self
    }

    /// Sets the case convention for struct field names. Defaults to [`Case::Snake`].
    pub fn field_case(mut self, case: Case) -> Self {
        self.field_case = Some(case);
        self
    }

    /// Sets the case convention for enum variant names. Defaults to
    /// [`Case::UpperSnake`].
    pub fn enum_member_case(mut self, case: Case) -> Self {
        self.enum_member_case = Some(case);
        self
    }

    /// Sets the case convention for constant names. Defaults to [`Case::UpperSnake`].
    pub fn constant_case(mut self, case: Case) -> Self {
        self.constant_case = Some(case);
        self
    }

    /// Renames a Rust type in the generated header.
    ///
    /// `c_name` is used verbatim: prefixes, suffixes and case conversion are all
    ...
    /// Builds the generator, applying the default for every option left unset.
    ///
    /// `crate_dir` is the directory holding the crate's `Cargo.toml`, whose `src`
    /// subdirectory is scanned.
    pub fn build(self, crate_dir: impl Into<PathBuf>) -> TerraffiGenerator {
        let config = TerraffiConfig {
            is_scan_dependencies: self.is_scan_dependencies.unwrap_or(true),
            exclude_crates: self.exclude_crates,
            assumed_declared_types: self.assumed_declared_types,
            is_allow_unresolved_types: self.is_allow_unresolved_types,

            header_comment: self.header_comment,
            header_guard: self.header_guard,
            export_macro: self.export_macro,

            is_add_std_includes: self.is_add_std_includes.unwrap_or(true),
            additional_includes: self.additional_includes,
            additional_macro_definitions: self.additional_macro_definitions,

            is_export_public_functions: self.is_export_public_functions.unwrap_or(true),
            is_export_public_types: self.is_export_public_types.unwrap_or(false),
            ...
            discriminant_enum_suffix: self
                .discriminant_enum_suffix
...
pub struct TerraffiConfig {
    ...
    /// Suffix appended to the discriminant enum name generated for a tagged union. Defaults to `"Kind"`.
    discriminant_enum_suffix: String,
...
/// Generates a C header from the source of a single crate.
///
/// Build one with [`TerraffiGeneratorBuilder`], then call
/// [`TerraffiGenerator::generate`].
pub struct TerraffiGenerator {
    ...
    unresolved_types: Vec<UnresolvedType>,
}
...
impl TerraffiGenerator {
    /// Creates a generator from an already-resolved configuration.
    ///
    /// Prefer [`TerraffiGeneratorBuilder`], which applies the defaults for you.
    ...
    fn discover_dependency_sources(&self) -> Result<Vec<DependencySource>, TerraffiError> {
        let manifest_path = self.crate_dir.join("Cargo.toml");
        let metadata = cargo_metadata::MetadataCommand::new()
            .manifest_path(&manifest_path)
            .exec()
            .map_err(|e| TerraffiError::CargoMetadata {
        ...
        let root_package = metadata
            .root_package()
            .ok_or_else(|| TerraffiError::NoRootPackage(manifest_path.clone()))?;
        ...
        let direct_dep_names: Vec<&str> = root_package
            ...
            .filter_map(|d| match d.name.as_str() {
                "terraffi" | "terraffi_ctypes" | "terraffi_gen" | "terraffi_helpers"
                | "terraffi_macro" => None,
        ...
        for dep_name in &direct_dep_names {
            ...
            let manifest_dir =
                package
                    .manifest_path
                    .parent()
                    .ok_or_else(|| TerraffiError::CargoMetadata {
    ...
    /// Types the last [`TerraffiGenerator::generate`] call referenced but did not declare.
    ///
    /// Always empty unless
    /// [`TerraffiGeneratorBuilder::allow_unresolved_types`] was set, since otherwise
    /// generation fails instead.
    pub fn unresolved_types(&self) -> &[UnresolvedType] {
        &self.unresolved_types
    }

    /// Generates the header, returning it as a string.
    ///
    /// # Errors
    ///
    /// Returns [`TerraffiError::UnresolvedTypes`] if the header would reference a
    /// type it does not declare, unless
    /// [`TerraffiGeneratorBuilder::allow_unresolved_types`] was set. Also fails if
    /// the crate directory cannot be read or its source cannot be parsed.
    pub fn generate(&mut self) -> Result<String, TerraffiError> {
        if !self.crate_dir.is_dir() {
            return Err(TerraffiError::CrateDirNotFound(self.crate_dir.clone()));
        }
        ...
        let real_enums: HashSet<_> = items
            ...
            .filter_map(|item| match item {
                CItem::Enum(e) if !e.is_synthesised_from_tagged_union => Some(e.name.clone()),
                _ => None,
            })
            .collect();

        items.retain(|item| match item {
            CItem::Enum(e) if e.is_synthesised_from_tagged_union => !real_enums.contains(&e.name),
            _ => true,
        });
        ...
        loop {
            ...
            for item in &items {
                ...
                match item {
                    ...
                    CItem::TaggedUnion(du) if self.referenced_types.contains(&du.name) => {
        ...
        // Slices and vectors have no single C spelling in a signature, so emitting
        // one would silently drop the length and mismatch the ABI.
        let unrepresentable = validate::find_unrepresentable(&relevant_items);
        if !unrepresentable.is_empty() {
            return Err(UnrepresentableTypesError { unrepresentable }.into());
        }

        // Reject references to types this header will not declare. Without this the
        // generator names them verbatim and the breakage only surfaces when the
        // consumer compiles their C.
        let unresolved =
            validate::find_unresolved(&relevant_items, &self.config.assumed_declared_types);
        if !unresolved.is_empty() {
            if !self.config.is_allow_unresolved_types {
                return Err(UnresolvedTypesError { unresolved }.into());
            }
            self.unresolved_types = unresolved;
        }
    ...
    fn emit_header(&mut self, w: &mut impl Write, items: &[CItem]) -> fmt::Result {
        ...
        // Names declared so far. A pointer to a type not yet in here is written
        // through its struct tag, which doubles as a forward declaration.
        ...
        for item in items {
            ...
            item.emit(w, &mut self.config, &declared)?;
            declared.insert(item.name().to_string());
        }
...
) -> Result<Vec<CItem>, TerraffiError> {
    if !src_dir.is_dir() {
        return Ok(Vec::new());
    }
...
fn scan_file(
    ...
    config: &TerraffiConfig,
    items: &mut Vec<CItem>,
) -> Result<(), TerraffiError> {
    let source = std::fs::read_to_string(path).map_err(|e| TerraffiError::ReadSource {
        path: path.to_path_buf(),
        source: e,
    })?;
    let file = syn::parse_file(&source).map_err(|e| TerraffiError::ParseSource {
        path: path.to_path_buf(),
        message: e.to_string(),
    })?;
    ...
    for item in &file.items {
        ...
        for i in
            CItem::from_item(item, source_order, config).map_err(TerraffiError::UnsupportedItem)?
        {
