...
impl CType {
    ...
    /// Like [`format`](Self::format), but for use *inside* the definition of the
    /// struct named `enclosing`. A pointer to the enclosing type itself is
    /// rendered with the `struct` tag keyword (e.g. `struct node_t*`) because the
    /// typedef alias is not yet in scope within the struct's own body — plain
    /// `node_t*` would not compile in C.
    /// Formats this type for use inside `enclosing`'s definition.
    ///
    /// A pointer to a struct that is not declared yet - the enclosing type
    /// itself, or one emitted later because the two form a reference cycle -
    /// is written through the struct tag. A pointer to an incomplete struct
    /// type is valid C, and `struct foo` serves as its forward declaration.
    fn format_within(
        &self,
        ...
        if let CType::Pointer { is_const, inner } = self
            && let CType::Named(name) = inner.as_ref()
            && (name == enclosing || !declared.contains(name))
        {
            let base = config.format_type_name_bare(name);
            return if *is_const {
                format!("const struct {base}*").into()
            } else {
                format!("struct {base}*").into()
            };
        }
        self.format(config)
    }

    /// Like [`format_named`](Self::format_named), but self-reference aware. See
    /// [`format_within`](Self::format_within).
    fn format_named_within(
        &self,
        name: &str,
        config: &TerraffiConfig,
        enclosing: &str,
        ...
        if let CType::Pointer { inner, .. } = self
            && let CType::Named(target) = inner.as_ref()
            && (target == enclosing || !declared.contains(target))
        {
            return format!(
                "{} {}",
                self.format_within(config, enclosing, declared),
                name
            );
        }
        self.format_named(name, config)
    }
...
/// If `ty` is `ManuallyDrop<T>` (or `core::mem::ManuallyDrop<T>` /
/// `std::mem::ManuallyDrop<T>`), returns the inner `T`. Otherwise returns `None`.
fn manually_drop_inner_type(ty: &Type) -> Option<&Type> {
    ...
    let segments: Vec<_> = type_path
        ...
        .collect();
    let is_manually_drop = match segments.as_slice() {
        [only] => only == "ManuallyDrop",
        [a, b] if b == "ManuallyDrop" => a == "std" || a == "core",
        [a, b, c] if c == "ManuallyDrop" => (a == "std" || a == "core") && b == "mem",
        _ => false,
    };
    if !is_manually_drop {
        return None;
...
impl TryFrom<&Type> for CType {
    ...
    fn try_from(value: &Type) -> Result<Self, Self::Error> {
        if let Some(inner) = manually_drop_inner_type(value) {
            return inner.try_into();
        }
        match value {
            Type::BareFn(bare_fn) => {
...
impl FromStr for CType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(match s {
            ...
            // `CStringBufferRef` has the same `{ ptr, len }` layout as
            // `CStringBuffer`, so it resolves to the same emitted struct. The
            // borrowed/owned distinction is a Rust-side concern, exactly as it is
            // for `CStringPtr` and `CStringPtrRef` above.
            "CStringBufferRef" => CType::Named("CStringBuffer".to_string()),
...
impl CItem {
            ...
            syn::Item::Enum(e) => {
                if is_repr_c(&e.attrs) {
                    ...
                    if has_data_variants {
                        match CTaggedUnion::from_item(e, source_order, config) {
                            Ok((e, du)) => Ok(vec![CItem::Enum(e), CItem::TaggedUnion(du)]),
                            Err(e) => Err(e),
                        }
                    } else {
    ...
    pub fn export_status(&self) -> TerraffiExportStatus {
        match self {
            ...
            CItem::TaggedUnion(u) => u.export_status,
    ...
    pub fn source_order(&self) -> usize {
        match self {
            ...
            CItem::TaggedUnion(u) => u.source_order,
    ...
    pub fn collect_referenced_type_names(&self, names: &mut HashSet<String>) {
        match self {
            ...
            CItem::TaggedUnion(u) => u.collect_referenced_type_names(names),
            CItem::Typedef(t) => {
    ...
    pub fn emit(
        &self,
        w: &mut impl Write,
        config: &mut TerraffiConfig,
        declared: &HashSet<String>,
    ) -> fmt::Result {
        match self {
            CItem::Struct(s) => s.emit(w, config, declared),
            CItem::Enum(e) => e.emit(w, config),
            CItem::TaggedUnion(u) => u.emit(w, config, declared),
...
impl CTypedef {
    ...
    pub fn emit(&self, w: &mut impl Write, config: &TerraffiConfig) -> fmt::Result {
        ...
        let tag_name = config.format_type_name_bare(&self.name);
        self.doc.emit_doxygen(w, "")?;
        match &self.inner_type {
            None => writeln!(w, "typedef struct {tag_name} {type_name};"),
...
/// A `#define` emitted into the generated header.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CMacro {
...
impl CMacro {
    ...
    ///
    /// `doc` is rendered above the `#define` as a Doxygen comment, and `definition`
    /// is emitted verbatim as the macro body.
    pub fn new(
...
impl CStruct {
    ...
    pub fn emit(
        ...
        let tag_name = config.format_type_name_bare(&self.name);
        self.doc.emit_doxygen(w, "")?;
        writeln!(w, "typedef struct {tag_name} {{")?;
        for field in &self.fields {
            field.emit(w, config, "    ", &self.name, declared)?;
        }
        writeln!(w, "}} {name};")
    }
}
    ...
    pub fn emit(
        ...
        if let CType::Slice { is_const, inner } = &self.ty {
            ...
            writeln!(
                ...
                "{}{} {};",
                indent,
                ptr_ty.format_within(config, enclosing, declared),
                self.name
            )?;
            ...
            writeln!(
                ...
                indent,
                self.ty
                    .format_named_within(&self.name, config, enclosing, declared)
            )
        }
    }
}
...
pub struct CEnum {
    ...
    pub is_synthesised_from_tagged_union: bool,
}
...
impl CEnum {
    pub fn from_item(e: &ItemEnum, source_order: usize) -> Result<Self, String> {
        ...
        Ok(CEnum {
            ...
            source_order,
            variants,
            is_synthesised_from_tagged_union: false,
        })
    }
    ...
    pub fn emit(&self, w: &mut impl fmt::Write, config: &mut TerraffiConfig) -> fmt::Result {
        ...
        self.doc.emit_doxygen(w, "")?;
        writeln!(w, "typedef enum {bare_name} {{")?;
...
impl CTaggedUnion {
    pub fn from_item(
        ...
        let discriminant_enum_name = match get_terraffi_discriminant_enum_name(&e.attrs) {
            ...
            None => format!("{}{}", name, config.discriminant_enum_suffix),
        };
        ...
        let discriminant_enum = CEnum {
            ...
            is_synthesised_from_tagged_union: true,
        };

        *source_order += 1;

        let union = CTaggedUnion {
            name,
            doc: doc.clone(),
            export_status,
            source_order: *source_order,
    ...
    pub fn emit(
        ...
        let tag_name = config.format_type_name_bare(&self.name);
        writeln!(w, "typedef struct {tag_name} {{")?;
        let tag_field_name = convert_case(&config.discriminant_enum_suffix, config.field_case);
            ...
            for variant in &variants_with_fields {
                    ...
                    )
                {
                    ...
                    writeln!(
                        ...
                        field
                            .ty
                            .format_named_within(&field_name, config, &self.name, declared),
                    )?;
                } else {
                    w.write_str("        struct {\n")?;
                    if variant.fields.len() == 1 {
                        ...
                        synthetic.emit(w, config, "            ", &self.name, declared)?;
                    } else {
