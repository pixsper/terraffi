...
impl CDoc {
    ...
    /// Builds a doc from a single block of text, used as the brief description.
    ...
    /// Writes the doc as a Doxygen block comment, prefixing each line with `indent`.
    ///
    /// Writes nothing when the doc is empty.
...
impl CFunctionDoc {
    ...
    /// Writes the doc as a Doxygen block comment, including an `@param` entry for
    /// each of `params` and an `@return` entry where the function returns a value.
    pub fn emit_doxygen(
        &self,
        w: &mut impl Write,
...
fn convert_rust_links_to_doxygen(text: &str) -> String {
    ...
    while i < len {
        ...
        // No link starts here: copy the whole character. `i` is always on a char
        // boundary (matches only ever consume ASCII-delimited spans), but the
        // character itself may be multi-byte, so it must not be pushed byte-wise.
