...
/// A nullable const pointer to `T`, laid out as a single pointer.
pub type RefPtr<'a, T> = Option<&'a T>;

/// A nullable mutable pointer to `T`, laid out as a single pointer.
pub type MutRefPtr<'a, T> = Option<&'a mut T>;

#[cfg(feature = "alloc")]
/// A nullable owning pointer to a heap-allocated `T`, laid out as a single pointer.
pub type BoxPtr<T> = Option<Box<T>>;

/// A FFI-safe handle to a heap-allocated `T`.
///
/// Exports as `T**` in C headers. The outer pointer indicates whether the
...
impl<T: Default> CHandle<T> {
    /// Allocates `T::default()` into the handle.
    ///
    /// # Errors
    ///
    /// See [`CHandle::alloc`].
