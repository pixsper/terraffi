...
use {
    ...
    alloc::string::{FromUtf8Error, FromUtf16Error, String},
    alloc::vec::Vec,
};
...
// Deserialization allocates, so these are only needed alongside `alloc`.
...
/// Writes a byte string to a formatter as UTF-8, substituting U+FFFD for any
/// invalid sequence.
///
/// Equivalent in output to formatting [`CStr::to_string_lossy`], but writes
/// directly to the formatter, so it needs neither an allocator nor an
/// intermediate buffer.
fn fmt_lossy(bytes: &[u8], f: &mut Formatter<'_>) -> fmt::Result {
    use core::fmt::Write;
    ...
    loop {
        match core::str::from_utf8(rest) {
            ...
            Err(e) => {
                ...
                // SAFETY: `from_utf8` reports the bytes before `valid_up_to` as valid UTF-8.
                f.write_str(unsafe { core::str::from_utf8_unchecked(&rest[..valid_up_to]) })?;
                f.write_char(char::REPLACEMENT_CHARACTER)?;
                match e.error_len() {
                    Some(len) => rest = &rest[valid_up_to + len..],
                    // Input ended mid-sequence; the replacement char covers the remainder.
                    None => return Ok(()),
                }
            }
        }
...
impl CStringPtr {
    ...
    ///
    /// This function will consume the provided data and use the underlying bytes to
    /// construct a new string, ensuring that there is a trailing 0 byte. If the provided
    /// data contains a null byte the string will be truncated to the previous character.
    ///
    #[cfg(feature = "alloc")]
    pub fn new_lossy<T: Into<Vec<u8>>>(t: T) -> Self {
        match CString::new(t) {
            Ok(s) => Self(unsafe { NonNull::new_unchecked(s.into_raw()) }),
            Err(e) => {
                let nul_pos = e.nul_position();
                let mut bytes = e.into_vec();
                bytes.truncate(nul_pos);
                // Safety: we truncated at the first null, so no null bytes remain.
                Self(unsafe {
                    NonNull::new_unchecked(CString::from_vec_unchecked(bytes).into_raw())
                })
            }
        }
    }
    ...
    pub fn as_bytes_with_nul(&self) -> &[u8] {
        unsafe { CStr::from_ptr(self.0.as_ptr()).to_bytes_with_nul() }
    }
...
impl Display for CStringPtr {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        fmt_lossy(unsafe { CStr::from_ptr(self.0.as_ptr()) }.to_bytes(), f)
    }
}
...
impl Clone for CStringPtr {
    fn clone(&self) -> Self {
        unsafe { Self::new(CStr::from_ptr(self.0.as_ptr()).to_bytes()).unwrap() }
    }
}
...
impl<'a> CStringPtrRef<'a> {
    ...
    /// trailing null byte.
    pub fn as_bytes_with_nul(&self) -> &[u8] {
        unsafe { CStr::from_ptr(self.ptr.as_ptr()).to_bytes_with_nul() }
    }
...
impl Display for CStringPtrRef<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        fmt_lossy(unsafe { CStr::from_ptr(self.ptr.as_ptr()) }.to_bytes(), f)
    }
}
...
impl CStringBuffer {
    /// Creates an `CStringBuffer` that holds an empty string (`""`).
    #[cfg(feature = "alloc")]
    ...
    /// Creates an `CStringBuffer` that holds the value of `str`.
    #[cfg(feature = "alloc")]
    pub fn new<'a>(str: impl Into<&'a str>) -> Self {
        str.into().into()
    }

    /// Creates an `CStringBuffer` representing the absent (`None`) state.
    ...
    /// Leaks the `CStringBuffer` and prevents `Drop` from being called.
    ///
    /// # Safety
    ///
    /// Should only be used to return the buffer to functions called from C, with the
    /// assumption that the C code will eventually call a rust function that calls `CStringBuffer::free`
    #[cfg(feature = "alloc")]
    pub unsafe fn leak(self) -> ManuallyDrop<Self> {
        ManuallyDrop::new(self)
    }

    /// Frees the memory in `CStringBuffer`
    ///
    /// # Safety
    ///
    /// Should only be used to free the memory of a buffer owned by a C caller.
    #[cfg(feature = "alloc")]
    ...
    /// Constructs an `CStringBuffer` from a raw pointer and byte length,
    ...
    /// Converts a vector of bytes to a `CStringBuffer`.
    #[cfg(feature = "alloc")]
    pub fn from_utf8(bytes: Vec<u8>) -> Result<CStringBuffer, FromUtf8Error> {
    ...
    /// Decodes a native-endian UTF-16 slice into a `CStringBuffer`.
    ///
    /// # Errors
    ///
    /// Returns `Err` if `buffer` is not valid UTF-16.
    #[cfg(feature = "alloc")]
    pub fn from_utf16(buffer: &[u16]) -> Result<CStringBuffer, FromUtf16Error> {
    ...
    /// Converts a slice of bytes to a `CStringBuffer`, replacing any invalid UTF-8 sequences with the Unicode replacement character.
    #[cfg(feature = "alloc")]
    pub fn from_utf8_lossy(bytes: &[u8]) -> CStringBuffer {
        String::from_utf8_lossy(bytes).into()
    }

    /// Decode a native endian UTF-16–encoded slice `buffer` `CStringBuffer`, replacing any invalid UTF-8 sequences with the
    /// Unicode replacement character.
    #[cfg(feature = "alloc")]
    pub fn from_utf16_lossy(buffer: &[u16]) -> CStringBuffer {
        String::from_utf16_lossy(buffer).into()
    }

    /// Converts a vector of bytes to a `CStringBuffer` without checking that the string contains valid UTF-8.
    ///
    /// # Safety
    ///
    /// The caller must ensure that the buffer is valid UTF-8.
    #[cfg(feature = "alloc")]
    pub unsafe fn from_utf8_unchecked(bytes: Vec<u8>) -> CStringBuffer {
    ...
    pub fn into_raw_parts(self) -> (*const c_char, usize) {
        // The caller takes ownership of the allocation, so this value's destructor
        // must not run. `Drop` is only implemented under `alloc`, and `ManuallyDrop`
        // states the requirement in either configuration.
...
#[cfg(feature = "alloc")]
impl<'a> From<Cow<'a, str>> for CStringBuffer {
...
impl CStringBufferRef<'_> {
    ...
    /// byte.
    pub fn as_bytes(&self) -> &[u8] {
        unsafe { slice::from_raw_parts(self.ptr.as_ptr() as *const u8, self.length_bytes.get()) }
    }
...
mod tests {
    ...
    /// `terraffi_gen` emits `CStringBufferRef` as the same C struct as
    /// `CStringBuffer`, which is only sound while their layouts agree.
    #[test]
    fn string_buffer_ref_matches_string_buffer_layout() {
        use core::mem::offset_of;

        assert_eq!(
            size_of::<CStringBuffer>(),
            size_of::<CStringBufferRef<'_>>(),
            "sizes differ"
        );
        assert_eq!(
            align_of::<CStringBuffer>(),
            align_of::<CStringBufferRef<'_>>(),
            "alignments differ"
        );
        assert_eq!(
            offset_of!(CStringBuffer, ptr),
            offset_of!(CStringBufferRef<'_>, ptr),
            "pointer is at a different offset"
        );
        assert_eq!(
            offset_of!(CStringBuffer, len),
            offset_of!(CStringBufferRef<'_>, length_bytes),
            "length is at a different offset"
        );
        // The niche keeps the borrowed form pointer+length sized inside an Option.
        assert_eq!(
            size_of::<Option<CStringBufferRef<'_>>>(),
            size_of::<CStringBuffer>(),
            "Option<CStringBufferRef> is not the same size"
        );
    }

    // --- lossy formatting ---

    /// `fmt_lossy` must agree with the standard library's lossy conversion on
    /// every input, valid or not.
    #[test]
    fn fmt_lossy_matches_from_utf8_lossy() {
        ...
        let cases: &[&[u8]] = &[
            ...
            &[0xf0, 0x9f, 0x92, 0x96], // valid 4-byte sequence
            &[0xff],                   // lone invalid byte
            &[b'a', 0xff, b'b'],       // invalid byte between valid ones
            &[0x80],                   // stray continuation byte
            &[0xe2, 0x82],             // truncated 3-byte sequence
            &[0xe2, 0x82, b'a'],       // 3-byte sequence cut short by ASCII
            &[0xf0, 0x9f, 0x92],       // truncated 4-byte sequence
            &[0xc0, 0x80],             // overlong encoding
            &[0xed, 0xa0, 0x80],       // surrogate half
            &[b'a', 0xff, 0xfe, b'b'], // consecutive invalid bytes
        ];

        for bytes in cases {
            let mut got = String::new();
            write!(got, "{}", Lossy(bytes)).unwrap();
            assert_eq!(
                got,
                String::from_utf8_lossy(bytes),
                "mismatch for bytes {bytes:?}"
            );
        }
    }
    ...
    // --- CStringBuffer leak / free ---

    #[test]
    fn cstring_buffer_leak_preserves_contents() {
        let s = CStringBuffer::new("hello");
        let ptr = s.ptr;
        let len = s.len;
        let mut leaked = unsafe { s.leak() };
        assert_eq!(leaked.ptr, ptr);
        assert_eq!(leaked.len, len);
        assert_eq!(leaked.as_bytes(), Some(&b"hello\0"[..]));
        // Drop the inner value to avoid a leak in the test process.
        unsafe { ManuallyDrop::drop(&mut leaked) };
    }

    #[test]
    fn cstring_buffer_leak_skips_drop() {
        // After leak, the original Drop impl must not run. We verify this by
        // observing that the underlying allocation is still valid.
        let s = CStringBuffer::new("persistent");
        let ptr = s.ptr;
        let len = s.len;
        let leaked = unsafe { s.leak() };
        // Read through the still-live allocation.
        let bytes = unsafe { core::slice::from_raw_parts(ptr as *const u8, len) };
        assert_eq!(&bytes[..len - 1], b"persistent");
        assert_eq!(bytes[len - 1], 0);
        // Reclaim manually so miri/leak detectors stay happy.
        let mut leaked = leaked;
        unsafe { ManuallyDrop::drop(&mut leaked) };
    }

    #[test]
    fn cstring_buffer_free_clears_pointer_and_len() {
        let mut s = CStringBuffer::new("free me");
    ...
    #[test]
    fn cstring_buffer_free_is_idempotent() {
        let mut s = CStringBuffer::new("twice");
        unsafe { s.free() };
        // A second free on the now-null buffer must be a no-op, not a double free.
        unsafe { s.free() };
        assert!(s.ptr.is_null());
        assert_eq!(s.len, 0);
    }
    ...
    #[test]
    fn cstring_buffer_free_on_none_is_noop() {
        let mut s = CStringBuffer::new_none();
        unsafe { s.free() };
        assert!(s.ptr.is_null());
        assert_eq!(s.len, 0);
    }
    ...
    #[test]
    fn cstring_buffer_leak_then_free_round_trip() {
        // Simulates the C round-trip: Rust leaks the buffer to C, C hands it
        // back, Rust frees it. Drop must not run on the leaked instance.
        let s = CStringBuffer::new("round trip");
        let leaked = unsafe { s.leak() };
        let mut returned = ManuallyDrop::into_inner(leaked);
        unsafe { returned.free() };
        assert!(returned.ptr.is_null());
