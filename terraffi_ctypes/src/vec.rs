...
impl<T> CVec<T> {
    ...
    /// Removes and returns the element at `index`, shifting everything after it to
    /// the left.
    ///
    /// # Panics
    ///
    ...
    /// element. Does not preserve order, but runs in O(1).
    ///
    /// # Panics
    ///
    /// Panics if `index >= len`.
    ...
    /// Shortens the vector to `len` elements, dropping the rest. Does nothing if
    /// `len` is already at or above the current length.
    #[cfg(feature = "alloc")]
    ...
    pub fn into_raw_parts(self) -> (*mut T, usize, usize) {
        ...
        (ptr, this.len, this.capacity)
    }

    #[cfg(feature = "alloc")]
    fn with_vec<R, F: FnOnce(&mut Vec<T>) -> R>(&mut self, f: F) -> R {
...
/// Gives `CVec<T>` the whole slice API, and with it indexing.
...
/// Consumes the vector, yielding its elements by value.
#[cfg(feature = "alloc")]
impl<T> IntoIterator for CVec<T> {
    ...
    fn into_iter(mut self) -> Self::IntoIter {
        // Move the allocation out of `self` so its `Drop` does not also free it.
        let taken = self.with_vec(core::mem::take);
        taken.into_iter()
    }
}

/// Appends every item from an iterator, mirroring `Vec`.
#[cfg(feature = "alloc")]
...
mod tests {
    ...
    /// The three raw fields must stay consistent through every mutation, including
    /// a reallocation that moves the pointer.
    #[test]
    fn push_across_a_reallocation_keeps_the_layout_consistent() {
        let mut v: CVec<u32> = CVec::with_capacity(1);
        v.push(1);
        ...
        assert_eq!(v.len(), 64);
        assert!(v.capacity() >= 64, "capacity should have grown");
        assert!(v.capacity() > first_cap, "expected a reallocation");
        assert_ne!(
            v.as_slice().as_ptr(),
            first_ptr,
            "the pointer should have moved, and been written back"
        );
        assert_eq!(v.as_slice(), (1..=64).collect::<alloc::vec::Vec<u32>>());
    }

    #[test]
    fn pop_returns_elements_and_shortens() {
        let mut v: CVec<u32> = vec![1, 2, 3].into();
    ...
    fn insert_and_remove_shift_elements() {
        let mut v: CVec<u32> = vec![1, 3].into();
        v.insert(1, 2);
    ...
    fn swap_remove_and_truncate() {
        let mut v: CVec<u32> = vec![1, 2, 3, 4].into();
        assert_eq!(v.swap_remove(0), 1);
        assert_eq!(v.len(), 3);
        v.truncate(1);
        assert_eq!(v.len(), 1);
        v.truncate(10);
        assert_eq!(v.len(), 1, "truncate beyond the length is a no-op");
    }

    #[test]
    fn deref_gives_indexing_and_slice_methods() {
        let mut v: CVec<u32> = vec![3, 1, 2].into();
        assert_eq!(v[0], 3);
        v[0] = 9;
        assert_eq!(v[0], 9);
    ...
    #[test]
    fn into_iterator_over_references() {
        let mut v: CVec<u32> = vec![1, 2, 3].into();
        assert_eq!((&v).into_iter().sum::<u32>(), 6);
        for x in &mut v {
            *x *= 2;
        }
    ...
    /// The owned iterator must take the allocation with it, so that `self`'s `Drop`
    /// does not also free it and the elements are not leaked either.
    #[test]
    fn into_iterator_by_value_transfers_the_allocation() {
        let v: CVec<u32> = vec![1, 2, 3].into();
        let collected: alloc::vec::Vec<u32> = v.into_iter().collect();
    ...
    /// Mutating an empty, never-allocated vector must not trip over the null pointer.
    #[test]
    fn mutation_from_empty_is_safe() {
        let mut v: CVec<u32> = CVec::new();
        assert_eq!(v.pop(), None);
        v.push(1);
    ...
    /// `into_raw_parts` hands the allocation to the caller and must suppress `Drop`.
    /// If it did not, rebuilding from the returned parts would be a use-after-free
    /// and the final drop a double free.
    #[test]
    fn into_raw_parts_transfers_ownership() {
        let original = CVec::from(vec![1u32, 2, 3, 4]);
        let (ptr, len, capacity) = original.into_raw_parts();

        assert!(!ptr.is_null());
        assert_eq!(len, 4);
        assert!(capacity >= 4);

        let rebuilt = unsafe { CVec::from_raw_parts(ptr, len, capacity) };
        assert_eq!(rebuilt.as_slice(), &[1, 2, 3, 4]);
        // `rebuilt` drops here, freeing the allocation exactly once.
    }

    #[test]
    fn into_raw_parts_of_empty_vec_yields_null() {
        let (ptr, len, capacity) = CVec::<u32>::default().into_raw_parts();
        assert!(ptr.is_null());
        assert_eq!(len, 0);
